...
ShapesDialog::ShapesDialog()
    :   timer(this), session(nullptr)
{
    //, pub(interop::detail::null<zenoh::Publisher>)
...
void ShapesDialog::setConfiguration(zenoh::Config config) 
{

    auto s = zenoh::Session::open(std::move(config));
    ...
    if(!conf)
    {
       ...
       // auto s = std::get<zenoh::Session>(zenoh::Session::open(std::move(zenoh::Config::create_default())));
        auto s = zenoh::Session::open(std::move(config));
        session = std::make_shared<zenoh::Session>(std::move(s));
    }
    else{
        ...
        //zenoh::Config config = std::get<zenoh::Config>(zenoh::from_file(conf));
        zenoh::Config config = zenoh::Config::from_file(conf);
         //auto s = std::get<zenoh::Session>(zenoh::Session::open(std::move(config)));
        auto s = zenoh::Session::open(std::move(config));
        session = std::make_shared<zenoh::Session>(std::move(s));
...
ShapesDialog::onPublishButtonClicked()
{
    ...
    switch (sIdx)
    {
        ...
        case CIRCLE:
        {
            ...
            zenoh::KeyExpr keyExp(keyExpression);
            //zenoh::Publisher p = std::get<zenoh::Publisher>(session->declare_publisher(keyExp));
            zenoh::Publisher p = session->declare_publisher(keyExp);
