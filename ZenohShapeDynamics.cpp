        ...
        dummy_(dummy)
{
    ...
    zenoh::KeyExpr keyexpr1(keyExpression);
  ...
  // subscriber_= std::get<zenoh::Subscriber>(session_->declare_subscriber(keyexpr1,[&] (const Sample& sample)// Capturing object by reference
    subscriber_= session_->declare_subscriber(keyexpr1,[&] (const Sample& sample)// Capturing object by reference
    {  
        //if (sample) {
       
        // Implementing Deserialize logic here
    	//void* ptr= (void*)sample.get_payload();
        auto ptr= sample.get_payload().as_vector();;
	    size_t sz = sample.get_payload().size();
        void* buffer = new char[sz];
        memcpy(buffer, ptr.data(), sz);
        ShapeType sT;
        if(deserialize_sample_from_buffer<ShapeType>(buffer, sz, sT, SDK_DATA))
        {
...
//subscriber_ = std::move(subscriber_1);
}
